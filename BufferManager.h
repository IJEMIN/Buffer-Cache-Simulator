...
public:
	...
	//Run 과 End 프로세스를 한큐에 묶어서 처리한다. While로 Run을 계속 돌리다가 Run이 수행되면 End로 빠진다.
	//해당 프로세스가 버퍼 취득 경쟁에서 진 상태라 판단되면 프로세스는 SLEEP으로 가고 false 리턴.
	bool ExecuteProcess(Process * runningProcess);

	//Run만 줄창 while로 돌리면서 end를 불리해낸다.
